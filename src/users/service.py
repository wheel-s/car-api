...
def change_password(
    db: Session, user_id: UUID, password_change: model.passwordChange
) -> None:
    try:
        ...
        if password_change.new_password != password_change.new_password_confirm:
            logging.warning(
                f"Invalid current password provided by user with user ID: {user_id}"
            )
        ...
        logging.error(
            f"Error during password change for user with ID: {user_id}. Error:{str(e)}"
        )
        raise

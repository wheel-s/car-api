...
def get_all_cars_by_brands(db:Session, brand_name:str, limit , page)->list[model.carResponse]:
   offset = limit *(page - 1) 
   brand = db.query(Brand).filter(Brand.name.ilike(f"%{brand_name}%")).first()
   cars = db.query(Car).filter(Car.brand_id == brand.id).offset(offset).limit(limit).all()
...
def get_car_and_brands_by_year(db:Session,year:int ,brand_name:str, limit , page):
   offset = limit *(page - 1) 
   brand = db.query(Brand).filter(Brand.name.ilike(f"%{brand_name}%")).first()
   cars = db.query(Car).filter(Car.brand_id == brand.id).filter(Car.year==year).offset(offset).limit(limit).all()
   logger.info(f"Retrieved all  car data")
   return cars

def get_spec_by_model(db:Session, model_name:str,limit,page):
   car = db.query(Car).filter(Car.model.ilike(f"%{model_name}%")).first()
...
def create_brand(db:Session, brand_request:model.createBrand, current_user:TokenData):
   try:
      brand_car = Car(
         id = uuid4(),
         user_id = current_user.get_uuid(),
         brand = brand_request.brand   
      )
      db.add(brand_car)
      db.commit()
      ...
      logger.error(f"Failed to create car {brand_request.brand}", extra={"Error":str(e)})
      raise HTTPException( status_code=500, detail=str(e))

def create_car(db:Session, car_request:model.carCreate, current_user:TokenData)->model.carResponse:
   try:
      new_car = Car(
         id = uuid4(),
         user_id = current_user.get_uuid(),
      ...
      logger.error(f"Failed to create car {car_request.model}", extra={"Error":str(e)})
      raise HTTPException( status_code=500, detail=str(e))


def update_car(db:Session,car_id:UUID, car_update:model.carUpdate, current_user:TokenData):
...
def create_Image(db:Session, image:model.carImageCreate, current_user:TokenData)->model.carImageResponse:
   try:
...
def update_image(db:Session, image_id:UUID, image_update:model.carImageUpdate, current_user:TokenData)->model.carImageResponse:
...
def append_image(db:Session, image_id:UUID, image_update:model.carImageUpdate, current_user:TokenData)->model.carImageResponse:
   image_data = image_update.model_dump(exclude_unset=True)
